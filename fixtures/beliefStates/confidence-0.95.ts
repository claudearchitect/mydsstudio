/**
 * Fixture: ~0.95 confidence — near end of session, all groups resolved and
 * sharp, ready for export. Used by A's "0.95 all sharp" gate and D's
 * export round-trip tests.
 */
import type { BeliefState } from "@/contracts";

export const confidence095: BeliefState = {
  schemaVersion: 1,
  meta: {
  ...
  groups: {
    ...
    spacing: {
      confidence: 0.9,
      tokens: {
        inset: { $value: "16px", $type: "dimension", provenance: ["e16"] },
      },
    },
  },
  rationale: [
    ...
    {
      id: "r05",
      claim: "gentle single-layer elevation, confirmed after seeing it on the card",
      tokens: ["elevation.card"],
      evidence: ["e13", "e15"],
    },
    {
      id: "r06",
      claim: "generous spacing to keep the booking flow feeling calm, not cramped",
  ...
  events: [
    ...
    {
      id: "e14",
      ts: "2026-07-03T00:01:29.000Z",
      kind: "interact",
      payload: { mode: "ask", question: "Does the card's shadow feel right, or too flat/heavy?" },
    },
    {
      id: "e15",
      ts: "2026-07-03T00:01:40.000Z",
      kind: "message",
      payload: { channel: "chat", text: "that's good, keep it subtle like that" },
    },
    {
      id: "e16",
      ts: "2026-07-03T00:01:45.000Z",
      kind: "update_beliefs",
      payload: { summary: "confirmed elevation, filled in generous spacing to match calm tone" },
    },
  ],
};
