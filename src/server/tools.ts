...
const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  [TOOL_NAMES.interact]:
    "Choose exactly one interaction for this turn: `mode: \"ask\"` for a natural-language question (with optional quick-reply chips — the client always also offers free text, so you do not need to engineer an escape hatch into the chips themselves; `quickReplies` is always an array, so use an empty array `[]` when you want no chips, never null), or `mode: \"propose\"` for a visual pick among 2-4 token-patch variants that vary a single named axis on one target component. Call this exactly once per turn, in parallel with exactly one `update_beliefs` call.",
